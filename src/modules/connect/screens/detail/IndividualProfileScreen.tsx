...
// Response keys specific to the individual profile endpoint:
//   GET /api/v1/individuals/public/individual-information/{uuid}
//   GET /api/v1/forms-management/profile/data/individual/{uuid}
...
export function IndividualProfileScreen({
  ...
  // ── individual-specific field mapping ────────────────────────────────────
  const designation = profile?.designation || profile?.jobTitle || '';
  const organization = profile?.organization || profile?.companyName || '';
  ...
  return (
    <ProfileShell
      ...
      {(expertise.length > 0 || industries.length > 0 || connectionRequirements.length > 0) ? (
        <SectionCard title="Interests & Expertise" primaryColor={primaryColor}>
          {expertise.length > 0 ? (
          ...
          {connectionRequirements.length > 0 ? (
            <>
              {(expertise.length > 0 || industries.length > 0) ? <Divider /> : null}
              <View style={sStyles.labelRow}>
